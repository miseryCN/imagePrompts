...
本 Skill 规范了向 `wkmdog` 提示词实验室（`imagePrompts`）提交新提示词、**清洗格式化整理提示词**、绑定图片/视频素材、自动计算标准画幅比例以及一键发布到 GitHub Pages 的标准作业流程。
...
## 2. 提示词格式化与清洗标准 (Prompt Formatting & Normalization)

> ⚠️ **核心原则**：录入提示词前，**必须先完成文本排版清洗**，杜绝直接粘贴带来的多余空行、乱跳行与粘连空格。

### 清洗规则清单：
1. **结构化要素分行排布**：
   - 凡带有要素标签（如 `核心主体：`、`衣着搭配：`、`表情妆容：`、`核心动作：`、`场景环境：`、`镜头景别：`、`光照氛围：`、`色彩风格：`、`整体氛围：`、`画质要求：`、`负面提示词：` 等），**必须各占独立一行**，禁止连续多个空格拼在一行。
2. **去除杂乱多余空格**：
   - 将行内连续的 2 个及以上多余空格收缩为 1 个空格；
   - 清除中文标点符号（`，`、`。`、`：`、`、`、`！`）前后无意义的半角空格；
   - 清除所有行首、行尾多余的空白字符。
3. **消除异常断行**：
   - 修复在复制过程中同一句话被硬回车打断的异常折行，保持语义连贯；
   - 合并连续多余的空白行（最多保留 1 个空行分隔自然段落）。
4. **禁止推荐模型字段**：
   - 严禁在 Markdown 或任何元数据字段中包含「推荐模型」信息。

---

## 3. Markdown 提示词文件标准结构
...
2. **文本清洗引擎**：二次校验并格式化所有 Markdown 提示词正文中的空格与换行。
3. **像素级比例计算**：通过 PIL 读取图片宽高，自动对齐最接近的标准比例（`21:9`, `16:9`, `3:2`, `4:3`, `1:1`, `3:4`, `2:3`, `9:16`, `9:21`）。
4. **时间戳提取与排序**：抓取精确创建/提交时间，按最新时间倒序排列（Newest First）。
5. **索引生成与双重缓存失效**：生成 `data/prompts.json` 与 `data/prompts.js`，并注入全新 Unix 时间戳版本号，彻底杜绝浏览器缓存。
6. **Git 自动化发布**：自动执行 `git add .`、`git commit` 并 `git push origin main` 到 GitHub Pages。
